...
namespace LibraryApp
{
    ...
    public partial class ModifyBookType : Window
    {
        ...
        public ModifyBookType()
        {
            InitializeComponent();
            InitializeTbStatus();
            BindingBookTypesToDGRDAsync();
        }
        private async Task<ObservableCollection<BookType>> GetBookTypes()
        {
            var rets = await bookTypeEntity.GetAllClientsAsync();
            bookTypes = new ObservableCollection<BookType>(rets);
            return bookTypes;
        }
        /// <summary>
        /// 将书籍类型绑定到DataGrid
        /// 订阅bookTypes集合更改事件
        /// </summary>
        private async void BindingBookTypesToDGRDAsync()
        {
            bookTypes = await GetBookTypes();
