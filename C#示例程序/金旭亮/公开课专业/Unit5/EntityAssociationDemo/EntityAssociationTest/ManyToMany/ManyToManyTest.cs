...
namespace EntityAssociationTest.ManyToMany
{
    [TestClass]
    public class ManyToManyTest
    {
        ...
        /// <returns></returns>
        [TestMethod]
        public async Task TestAddUserAndRoleAllInMemory()
        {
        ...
        public async Task TestAddUserToRole()
        {
            using (var context = new MyDBEntities())
            {
                ...
                if (admin != null)
                {
                    ...
                    //验证用户己经加入到系统管理员角色中……
                    Role adminFromDB = await context.Roles
                                    .FirstOrDefaultAsync(
                                            r => r.RoleName == "SystemAdmin"
                                    );
        ...
        public async Task TestAssignRoleToUser()
        {
            using (var context = new MyDBEntities())
            {
                Role admin =await context.Roles.FirstOrDefaultAsync(
                    r => r.RoleName == "SystemAdmin");
                Role guest =await context.Roles.FirstOrDefaultAsync(
                    r => r.RoleName == "Guest");
        ...
        /// <returns></returns>
        [TestMethod]
        public async Task DeleteSystemAdminRole()
        {
            using (var context = new MyDBEntities())
            {
                ...
                if (systemAdmin != null)
                {
                   ...
                   //确认用户表中的记录仍然存在
                    ...
                    for (int i = 0; i < users.Count; i++)
                    {
                        ...
                        temp = await context.Users.FirstOrDefaultAsync(
                            u => u.UserId == id);
