...
namespace IntroduceParallel
{
    class Program
    {
        ...
        {
            //FirstUseParallelInvoke();
            FirstUseParallelLoop();
            Console.ReadLine();
        }
        #region "并行执行"
        static void DoWork1()
        ...
        static void FirstUseParallelInvoke()
        {
            Console.WriteLine("主线程{0}启动并执行",Thread.CurrentThread.ManagedThreadId);
            Parallel.Invoke(
                () => DoWork1(),
                () => DoWork2(),
                () => DoWork3()
        ...
        private static void FirstUseParallelLoop()
        {
